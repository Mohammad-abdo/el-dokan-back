...
<head>
    ...
    <meta charset="UTF-8">
    <title>Shop Report — {{ $report['shop']['name'] ?? 'Shop' }}</title>
    <style>
...
<body>
    ...
    @php $shop = $report['shop'] ?? []; @endphp
    <div class="shop-card">
        <div class="shop-card-top">
            <div class="shop-avatar">
                {{ mb_strtoupper(mb_substr($shop['name'] ?? 'S', 0, 1)) }}
            </div>
            <div class="shop-info">
                ...
                <div class="shop-meta">
                    ...
                    <div><strong>Address:</strong> {{ Str::limit($shop['address'] ?? '-', 80) }}</div>
                    @if(isset($shop['vendor_status']))
                        <div><strong>Status:</strong> {{ $shop['vendor_status'] }}</div>
                    @endif
                </div>
            </div>
        </div>
    ...
    @if(isset($report['sections']['overview']))
        @php $ov = $report['sections']['overview']; @endphp
            ...
            <div class="section-body">
                @if(!empty($ov['summary']))
                    <table class="kv-table">
                        <tbody>
                            @foreach($ov['summary'] as $k => $v)
                                <tr>
                                    <td>{{ $k }}</td>
                                    <td>{{ is_scalar($v) || $v === null ? ($v ?? '-') : Str::limit(json_encode($v, JSON_UNESCAPED_UNICODE), 120) }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if(!empty($p['first_image_url']))
                                            <img class="img-thumb" src="{{ $p['first_image_url'] }}" alt="img" />
                                        @else
                                            -
                                        @endif
                                    </td>
    ...
    @if(isset($report['sections']['wallet']))
        @php $w = $report['sections']['wallet']; @endphp
        <div class="section">
            <div class="section-header">
                <span class="section-title">Wallet &amp; Transactions</span>
                <span class="section-count">{{ $w['transactions_count'] ?? 0 }} transactions</span>
            </div>
            <div class="section-body">
                <table class="kv-table">
                    <tbody>
                        @foreach([
                            ...
                            <tr>
                                <td>{{ $label }}</td>
                                <td>{{ isset($w[$key]) ? $w[$key] : '-' }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>

                @if(!empty($w['transactions']))
                    <table class="data-table">
                        <thead>
                            <tr>
                                <th>#</th>
    ...
    {{-- ── ORDERS FROM REPS ── --}}
    @if(!empty($report['sections']['ordersFromReps']['orders']))
        @php $b = $report['sections']['ordersFromReps']; @endphp
        <div class="section">
            <div class="section-header">
                <span class="section-title">Orders From Reps</span>
                <span class="section-count">{{ $b['total'] ?? count($b['orders'] ?? []) }} orders</span>
            </div>
            <div class="section-body">
                <table class="data-table">
                    <thead>
    ...
    {{-- ── VISITS ── --}}
    @if(!empty($report['sections']['visits']['visits']))
        @php $b = $report['sections']['visits']; @endphp
        <div class="section">
            <div class="section-header">
                <span class="section-title">Visits</span>
                <span class="section-count">{{ $b['total'] ?? count($b['visits'] ?? []) }} visits</span>
            </div>
            <div class="section-body">
                <table class="data-table">
                    <thead>
    ...
    {{-- ── REPRESENTATIVES ── --}}
    @if(!empty($report['sections']['representatives']['representatives']))
        @php $b = $report['sections']['representatives']; @endphp
        <div class="section">
            <div class="section-header">
                <span class="section-title">Representatives</span>
                <span class="section-count">{{ $b['total'] ?? count($b['representatives'] ?? []) }} reps</span>
            </div>
            <div class="section-body">
                <table class="data-table">
                    <thead>
                    ...
                    <tbody>
                        @foreach(($b['representatives'] ?? []) as $i => $r)
                            @php
                                $u = $r['user'] ?? [];
                            @endphp
                            <tr>
                                ...
                                <td>{{ $u['username'] ?? '-' }}</td>
                                <td>{{ $u['phone'] ?? '-' }}</td>
                                <td>{{ Str::limit($r['territory'] ?? '-', 40) }}</td>
    ...
    {{-- ── COMPANY ORDERS ── --}}
    @if(!empty($report['sections']['companyOrders']['orders']))
        @php $b = $report['sections']['companyOrders']; @endphp
        <div class="section">
            <div class="section-header">
    ...
    {{-- ── BRANCHES ── --}}
    @if(!empty($report['sections']['branches']['branches']))
        @php $b = $report['sections']['branches']; @endphp
        <div class="section">
            <div class="section-header">
                <span class="section-title">Branches</span>
                <span class="section-count">{{ count($b['branches'] ?? []) }} branches</span>
            </div>
            <div class="section-body">
                <table class="data-table">
                    <thead>
                    ...
                    <tbody>
                        @foreach(($b['branches'] ?? []) as $i => $br)
                            <tr>
                                <td>{{ $i + 1 }}</td>
                                <td>{{ $br['name'] ?? '-' }}</td>
                                <td>{{ $br['name_ar'] ?? '-' }}</td>
                                <td>{{ Str::limit($br['address'] ?? '-', 45) }}</td>
                                <td>{{ $br['phone'] ?? '-' }}</td>
                                <td>{{ isset($br['is_active']) ? ($br['is_active'] ? '&#10003;' : '&times;') : '-' }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
    ...
    {{-- ── DOCUMENTS ── --}}
    @if(!empty($report['sections']['documents']['documents']))
        @php $b = $report['sections']['documents']; @endphp
        <div class="section">
            <div class="section-header">
                <span class="section-title">Documents</span>
                <span class="section-count">{{ count($b['documents'] ?? []) }} docs</span>
            </div>
            <div class="section-body">
                <table class="data-table">
                    <thead>
                    ...
                    <tbody>
                        @foreach(($b['documents'] ?? []) as $i => $d)
                            <tr>
                                ...
                                <td>{{ isset($d['issue_date']) ? \Carbon\Carbon::parse($d['issue_date'])->format('M d, Y') : '-' }}</td>
                                <td>{{ isset($d['expires_at']) ? \Carbon\Carbon::parse($d['expires_at'])->format('M d, Y') : '-' }}</td>
                                <td>{{ isset($d['is_verified']) ? ($d['is_verified'] ? '&#10003;' : '&times;') : '-' }}</td>
                                <td>{{ !empty($d['file_url']) ? Str::limit($d['file_url'], 28) : '-' }}</td>
                                <td>{{ Str::limit($d['notes'] ?? '-', 35) }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
    ...
    <div class="footer">
        <div class="footer-left">
            ...
            Shop: {{ $shop['name'] ?? '-' }} &bull; ID: {{ $shop['id'] ?? '-' }}
        </div>
